(function($) {
	...
	function isotopeConfig() {
		...
		$(window).on("load resize", function() {
			...
			$pages.on('click touchstart', 'li', function(e){
				e.preventDefault();
				var selector = jQuery(this).find("a").attr('data-filter');
				curIsotopePage = selector;
				$('#portfolio_iso_pages_current').html(selector.substr(selector.lastIndexOf('_')+1));
				$container.isotope({ filter: getIsotopeFilter() });
				$(this).find("a").parents('#portfolio_iso_pages').find('a').removeClass('current');
			...
			if($form.valid()) {
				$.ajax({
					...
					complete: function() {
						...
						$form.find("#cform-submit").parent().after($(response).fadeIn(1000).delay(1000).fadeOut(function(){$(this).remove()}));
					},
					dataType: 'json'
				});
			}
	...
	function internalMenu() {
		...
		// Show menu on click
		$innerNav.find(".menu-btn").on("click", function(e) {
			if (e.clientX > $(this).offset().left + 60 && e.clientY < $(this).offset().top + 60) {
				
			}else{
				$('.menu-content').stop().toggleClass("show-menu");
			}
			e.preventDefault();
			/*
			if($(".menu-content").hasClass("show-menu")){
				$(".menu-content").removeClass("show-menu");
			}else{
